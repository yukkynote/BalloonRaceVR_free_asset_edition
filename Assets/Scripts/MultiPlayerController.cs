...
public class MultiPlayerController:MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    bool startSoundFlg = false; // タイムオーバーフラグ
    ...
    public int distanceRecordData;
    // PlayFab
    string selfPlayerId; // プレイヤーID
    [SerializeField] GetPlayerCombinedInfoRequestParams InfoRequestParams; // リクエストパラメータ
    ...
    void Start() {
        ...
        // Avatar内のパーティクルオブジェクトを取得
        flame1 = GameObject.Find("Flame 1");
        flame2 = GameObject.Find("Flame 2");
        if (flame1 != null) {
            particle1 = flame1.GetComponent<ParticleSystem>();
        }
        if (flame2 != null) {
            particle2 = flame2.GetComponent<ParticleSystem>();
        }
        // ゲームオブジェクトを検索
        goalTarget = GameObject.Find("GoalTarget"); // ターゲット
        LeftHandAnchor = GameObject.Find("LeftHandAnchor"); // 左手アンカー
        RightHandAnchor = GameObject.Find("RightHandAnchor"); // 右手アンカー
        RIndex = GameObject.Find("hands:b_r_index3"); // 右手指先
        LHandIndex = GameObject.Find("coll_hands:b_l_index3"); // 左手指先
        RHandIndex = GameObject.Find("hands_coll:b_r_index3"); // 右手指先
        watchL = GameObject.Find("WatchL"); // 左手腕時計
        watchR = GameObject.Find("WatchR"); // 右手腕時計
        // メニュー初期設定
        InitMenu();
        // バーナーの炎パーティクルの初期化
        particle1.Stop();
        ...
        // クリアタイム初期化
        clearTime = 0;
        distanceRecordData = 999999;
        // ミニマップの経路オブジェクト格納配列初期化
        routes = new GameObject[310];
        enemyRoutes = new GameObject[310 * 4];
        //　goalPos = GameObject.FindGameObjectWithTag("Finish").transform.position;
        ...
        for (var i = 0; i < 25; i++) {
            for (var j = 0; j < 10; j++) {
                for (var k = 0; k < 25; k++) {
                    // 高度によって風の向きと強さを設定
                    windArray[i, j, k] = windPowers[j];
                    float yRot = (float)((windArray[i, j, k] % 10) * 45); // Y軸の角度
    ...
    void Update() {
        ...
        if (isPlaying) {
            ...
            if (!isPause) {
                ...
                if ((!isGoal && isMarkerDrop && markerController.isGroundEnter()) || isOver) {
                    ...
                    if (!isOver) {
                        ...
                        // Photonにスコアデータを送信
                        if (photonView.IsMine) {
                            photonView.RPC(nameof(ScoreUpdate), RpcTarget.All, distanceRecordData, PhotonNetwork.LocalPlayer.ActorNumber);
                        }
                ...
                // 右手トリガーのいずれかが離された時
                // もしくはUnityエディタモードでEnterキーが押されたとき
            ...
            if (Input.GetKey(KeyCode.Escape)) {
...
#endif
                // 上記以外でいずれかの左トリガーが押されていれば(右トリガー同時押しの場合も含む)※Unityエディタの場合は下矢印を押している時も
            } else if (OVRInput.Get(OVRInput.RawButton.LIndexTrigger) || OVRInput.Get(OVRInput.RawButton.LHandTrigger)
#if UNITY_EDITOR
                || (Input.GetKey(KeyCode.DownArrow))
#endif
            ) {
                // 左のロープ(リップライン)に左手の指先が入っていれば※Unityエディタの場合は下矢印を押している時も
                ...
                // 左トリガーが押されておらず右トリガーが押されていれば※Unityエディタの場合は上矢印を押している時も
            } else if (
                OVRInput.Get(OVRInput.RawButton.RIndexTrigger) || OVRInput.Get(OVRInput.RawButton.RHandTrigger)
#if UNITY_EDITOR
                ...
                // バーナーレバーのボタンに右の指先が入っていれば※Unityエディタの場合は上矢印を押している時も
                if (burnerController.isBurnerHand()
#if UNITY_EDITOR
                    || (Input.GetKey(KeyCode.UpArrow))
#endif
            ...
            if (intervalActionFlg) {
                ...
                foreach (GameObject avatar in avatars) {
                    // プレイヤー自身のゲームオブジェクトでなければ
                    if (avatar.GetInstanceID() != this.gameObject.GetInstanceID()) {
                        // ミニマップのライバルプレイヤーの座標を同期する
                        miniEnemyBalloons[i].transform.localPosition = new Vector3(
                            (avatar.transform.position.x + 0.0f) / 1000.0f - 0.0f + 0.0f,
                            avatar.transform.position.y / 1000.0f + 0.0f,
                            (avatar.transform.position.z + 0.0f) / 1000.0f - 0.0f + 0.0f);
            ...
            if (secondTimer >= 2.0f && sec < 1200) {
                ...
                foreach (GameObject avatar in avatars) {
                    ...
                    if (avatar.GetInstanceID() != this.gameObject.GetInstanceID()) {
                        ...
                        enemyRoutes[sec * j] = Instantiate(enemyRoutePrefab, Vector3.zero, Quaternion.identity);
                        enemyRoutes[sec * j].transform.parent = miniMap.transform;
                        enemyRoutes[sec * j].transform.position = Vector3.zero;
                        enemyRoutes[sec * j].transform.localPosition = new Vector3(
                            (avatar.transform.position.x + 0.0f) / 1000.0f - 0.0f + 0.0f,
                            avatar.transform.position.y / 1000.0f + 0.0f,
                            (avatar.transform.position.z - 0.0f) / 1000.0f - 0.0f + 0.0f
                        );
                        enemyRoutes[sec * j].transform.localScale = new Vector3(1 / 500.0f, 1 / 500.0f, 1 / 500.0f);
                        j++;
                    }
                }
                // マップの経路表示のタイミングをリセット
                ...
                if (timeElapsed <= 0.0f) {
                    ...
                    foreach (GameObject avatar in avatars) {
                        ...
                        if (avatar.GetInstanceID() != this.gameObject.GetInstanceID()) {
                            ...
                            miniEnemyBalloons[i].transform.localPosition = new Vector3(
                                (avatar.transform.position.x + 0.0f) / 1000.0f - 0.0f + 0.0f,
                                (avatar.transform.position.y + 0.0f) / 1000.0f + 0.0f,
                                (avatar.transform.position.z + 10.0f) / 1000.0f - 0.0f + 0.0f
                            );
                            miniEnemyBalloons[i].transform.localScale = new Vector3(1 / 50.0f, 1 / 50.0f, 1 / 50.0f);
        ...
        if (isGoal || isOver) {
            ...
            if (currentMenuNo != 2) {
                ...
                if (afterTimeElapsed >= 1.0f) {
                    ...
                    foreach (var player in PhotonNetwork.PlayerList) {
                        // 有効な値のスコアが記録されていれば
                        if (player.GetScore() != 0 && player.GetScore() != 999999) {
                            // 結果表示に追加
                            resultsTxtTmp.text += $"{player.NickName}({player.ActorNumber}) {((float)player.GetScore() / 1000).ToString("F3")}m";
                            // 自分の場合は区別して表示
                            if (PhotonNetwork.LocalPlayer.ActorNumber == player.ActorNumber) {
                                resultsTxtTmp.text += "(You)";
                            }
                            // 改行コードで改行する
                            resultsTxtTmp.text += "\n";
                        }
            ...
            // XYZ方向の風向きを加速度に変換
            xVelocity = xWind * Time.deltaTime * 3.0f;
            yVelocity = yWind * Time.deltaTime * 3.0f;
            zVelocity = zWind * Time.deltaTime * 3.0f;
            // 現在座標から風向き方向に加速させる
            pos.x += xVelocity * Time.deltaTime * 4.0f;
            pos.y += yVelocity * Time.deltaTime * 4.0f;
            pos.z += zVelocity * Time.deltaTime * 4.0f;
            // 現在座標を更新
            myTransform.position = pos;
            // 定期的な実行のタイミングであれば
            if (intervalActionFlg) {
    ...
    [PunRPC]
    private void ScoreUpdate(int score, int ownerId) {
        // 所有者IDがプレイヤー自身のActorNumberと一致すれば
    ...
    void OnEnable() {
        ...
        GetPlayerData();
        // Photonから自身のActorNoを取得
        int id = photonView.OwnerActorNr;
        // ActorNoを4で割った余り0～3で分岐する
        // Avatar内の4つのバルーンオブジェクトのいずれかを有効にする
        switch (id % 4) {
            case 1:
        ...
        // idに対応する開始位置に移動させる
        Vector3 pos = GameObject.Find("startPos" + id).transform.position;
        if (pos != null) {
    ...
    /// <summary>
    /// Photon経由で各プレイヤーの座標と距離スコアを送受信する処理
    /// </summary>
    /// <param name="stream">PhotonStreamオブジェクト</param>
    /// <param name="info">PhotonMessageInfoオブジェクト</param>
    void IPunObservable.OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info) {
        if (stream.IsWriting) {
            ...
            // 自身の距離スコアを送信する
            if (distanceRecordData != 0.0f) {
                stream.SendNext(distanceRecordData);
            }
        } else {
            // 座標情報を取得して適用
            ...
            // 他プレイヤーの距離スコアを受信する
            distanceRecordData = (int)stream.ReceiveNext();
        }
    }
